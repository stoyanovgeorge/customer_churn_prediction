...
def train_random_forest_classifier(
    ...
    # Saving all data metrics into a dedicated variable
    data_metrics = mlflow.active_run().data.metrics
    # Extracting the accuracy or returning 0 
    accuracy = data_metrics.get("accuracy", 0.0)

    # Extracting the MLflow Run ID
    run = mlflow.active_run()
    mlflow_run_id = run.info.run_id

    # Return all autologged metrics for comparison    
    return {
        "all_metrics": data_metrics,
